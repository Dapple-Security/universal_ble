      ...
      const std::function<void(ErrorOr<bool> reply)> result)
  {
    try
    {
      ...
      else
      {
        ...
        std::cout << "PairLog: Received pairing status: " << static_cast<int>(pair_result.Status()) << std::endl;
        ...
        if (error_str.has_value())
        {
            ...
            std::cout << "PairLog: Pairing failed: " << error_str.value() << std::endl;
        }
    ...
    {
      std::cout << "PairLog: Exception: " << ex.what() << std::endl;
      result(false);
    }
    catch (...)
    {
